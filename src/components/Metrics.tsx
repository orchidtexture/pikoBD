...
export const Metrics: React.FC<MetricsProps> = ({ stats }) => {
  const metricsData = [
    ...
    { 
      label: 'Weekly Status Checks', 
      value: stats.weeklyStatusChecks, 
      icon: Zap, 
      color: 'text-indigo-600', 
      bg: 'bg-indigo-50',
      goal: stats.totalClients,
      showProgress: true,
      progressLabel: 'Client Coverage'
    },
    { 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {metricsData.map((stat, i) => (
          <motion.div 
            ...
            key={stat.label} 
            className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3 transition-colors duration-300"
          >
                ...
                <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <motion.div 
                    ...
                    className={cn(
                      ...
                      stat.value >= stat.goal ? "bg-emerald-500" : "bg-indigo-500"
                    )}
                  />
                </div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex justify-between">
                  <span>{stat.progressLabel}</span>
                  <span className={cn(stat.value >= stat.goal ? "text-emerald-600" : "text-indigo-600")}>
